...
def test(args, model, code_tokenizer, text_tokenizer, eval_when_training=False):
    # Build dataloader
    ...
    for batch in tqdm(test_dataloader):
        ...
        with torch.no_grad():
            code_embedding, description_0_embedding = \
                model(func_input_ids, func_attention_mask, description_0_input_ids, description_0_attention_mask,
                      flag="test")
            _, description_1_embedding = \
                model(func_input_ids, func_attention_mask, description_1_input_ids, description_1_attention_mask,
                      flag="test")
            similarity0 = compute_similarity(code_embedding, description_0_embedding)
            similarity1 = compute_similarity(code_embedding, description_1_embedding)
            similarity = torch.stack([similarity0, similarity1], dim=-1)
...
def test_cls(args, model, code_tokenizer, text_tokenizer, eval_when_training=False):
    # Build dataloader
        ...
        with torch.no_grad():
            ...
            for i in range(len(description_input_ids_list)):
                ...
                code_embedding, description_embedding = \
                    model(func_input_ids, func_attention_mask, description_input_ids, description_attention_mask,
                          flag="test")
                similarity = compute_similarity(code_embedding, description_embedding)
                similarity_all.append(similarity)
            similarity_all = torch.stack(similarity_all, dim=1)
            preds = similarity_all.argmax(dim=-1)
            y_preds.append(preds.cpu().numpy())
            y_trues.append(label.cpu().numpy())
        nb_eval_steps += 1
...
def main():
    ...
    parser.add_argument("--do_linear_probe_detection", action='store_true',
                        help="Whether to run training.")
    ...
    total_num = sum(p.numel() for p in vul_model.parameters())
    ...
    results = {}
    if args.do_test:
        checkpoint_prefix = args.to_checkpoint + '/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        vul_model.load_state_dict(torch.load(output_dir))
        vul_model.to(args.device)
        result=test(args, vul_model, code_tokenizer, text_tokenizer)
    ...
    if args.do_test_cls:
        ...
        vul_model.load_state_dict(torch.load(output_dir))
        vul_model.to(args.device)
        result = test_cls(args, vul_model, code_tokenizer, text_tokenizer)
        print("Test result: ", result)
